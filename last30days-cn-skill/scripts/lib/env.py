...
def get_config() -> Dict[str, Any]:
    ...
    # Track WEIBO_SUB source for diagnostics.
    if config.get('WEIBO_SUB'):
        if os.environ.get('WEIBO_SUB') or merged_env.get('WEIBO_SUB'):
            config['_WEIBO_SUB_SOURCE'] = 'env'
        elif browser_creds.get('WEIBO_SUB'):
            browser_name = browser_creds.get('__WEIBO_BROWSER', 'unknown')
            config['_WEIBO_SUB_SOURCE'] = f'browser-{browser_name}'
        else:
            config['_WEIBO_SUB_SOURCE'] = 'env'
    else:
        config['_WEIBO_SUB_SOURCE'] = None
...
def is_weibo_available(config: Dict[str, Any]) -> bool:
    ...
    Requires WEIBO_SUB cookie (and optionally WEIBO_SUBP).
    """
    return bool(config.get('WEIBO_SUB'))
...
def is_douyin_available(config: Dict[str, Any]) -> bool:
    ...
    Douyin hot-search endpoint is public and does not require auth keys.
    """
    return True
