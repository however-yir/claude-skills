...
def _xref_tag(item) -> str:
    ...
    for ref_id in refs:
        ...
        elif ref_id.startswith('XHS'):
            source_names.add('Xiaohongshu')
        elif ref_id.startswith('WB'):
            source_names.add('Weibo')
        elif ref_id.startswith('DY'):
            source_names.add('Douyin')
...
def render_source_status(report: schema.Report, source_info: dict = None) -> str:
    ...
    if report.web:
        ...
        wb_count = sum(
            1 for w in report.web
            if getattr(w, "source_domain", "").lower().endswith("weibo.com")
        )
        dy_count = sum(
            1 for w in report.web
            if getattr(w, "source_domain", "").lower().endswith("douyin.com")
        )
    if xhs_count > 0:
    ...
    if wb_count > 0:
        lines.append(f"  ✅ Weibo: {wb_count} posts")
    else:
        reason = source_info.get("weibo_skip_reason")
        if reason:
            lines.append(f"  ⚡ Weibo: {reason}")
    if dy_count > 0:
        lines.append(f"  ✅ Douyin: {dy_count} topics")
    else:
        reason = source_info.get("douyin_skip_reason")
        if reason:
            lines.append(f"  ⚡ Douyin: {reason}")
