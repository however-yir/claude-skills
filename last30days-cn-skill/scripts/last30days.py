...
TIMEOUT_PROFILES = {
    "quick":   {"global": 90,  "future": 30, "reddit_future": 60,  "youtube_future": 60,  "tiktok_future": 90,   "instagram_future": 90,   "hackernews_future": 30,  "bluesky_future": 30,  "truthsocial_future": 30,  "weibo_future": 30,  "douyin_future": 30,  "polymarket_future": 15,  "http": 15, "enrich_per": 8,  "enrich_total": 30, "enrich_max_items": 10},
    "default": {"global": 180, "future": 60, "reddit_future": 90,  "youtube_future": 90,  "tiktok_future": 120,  "instagram_future": 120,  "hackernews_future": 60,  "bluesky_future": 60,  "truthsocial_future": 60,  "weibo_future": 60,  "douyin_future": 60,  "polymarket_future": 30,  "http": 30, "enrich_per": 15, "enrich_total": 45, "enrich_max_items": 15},
    "deep":    {"global": 300, "future": 90, "reddit_future": 120, "youtube_future": 120, "tiktok_future": 150,  "instagram_future": 150,  "hackernews_future": 90,  "bluesky_future": 90,  "truthsocial_future": 90,  "weibo_future": 90,  "douyin_future": 90,  "polymarket_future": 45,  "http": 30, "enrich_per": 15, "enrich_total": 60, "enrich_max_items": 25},
}
...
VALID_SEARCH_SOURCES = {
    ...
    "polymarket", "web", "xiaohongshu", "xhs", "weibo", "douyin",
}
...
def _search_weibo(
    ...
    """Search Weibo via native API connector (runs in thread).

    Returns:
        Tuple of (weibo_items, weibo_error)
        Items are in web-item dict shape and can be normalized with websearch module.
    """
    sub = config.get("WEIBO_SUB") or ""
    subp = config.get("WEIBO_SUBP") or ""
    ...
    return items, response.get("error")
...
def _search_douyin(
    ...
    """Search Douyin via native hot-search connector (runs in thread).

    Returns:
        Tuple of (douyin_items, douyin_error)
        Items are in web-item dict shape and can be normalized with websearch module.
    """
    response = douyin.search_douyin(topic, from_date, to_date, depth=depth)
    items = response.get("items", [])
    return items, response.get("error")
...
def run_research(
    ...
    def _append_web_error(current: str, new_msg: str) -> str:
        if not new_msg:
            return current
        if not current:
            return new_msg
        return f"{current}; {new_msg}"
    ...
    web_needed = do_web and not web_backend and not (run_xiaohongshu or run_weibo or run_douyin)
    ...
    if sources == "web":
        ...
        if run_xiaohongshu:
            try:
                ...
                if xiaohongshu_error:
                    web_error = _append_web_error(web_error, f"Xiaohongshu: {xiaohongshu_error}")
            except Exception as e:
                xiaohongshu_error = f"{type(e).__name__}: {e}"
                if progress:
                    progress.show_error(f"Xiaohongshu error: {e}")
                web_error = _append_web_error(web_error, f"Xiaohongshu: {xiaohongshu_error}")
        # Optional Weibo search in web-only mode.
        if run_weibo:
            try:
                weibo_items, weibo_error = _search_weibo(
                    topic, config, from_date, to_date, depth,
                )
                web_items.extend(weibo_items)
                if weibo_error and progress:
                    progress.show_error(f"Weibo error: {weibo_error}")
                if weibo_error:
                    web_error = _append_web_error(web_error, f"Weibo: {weibo_error}")
            except Exception as e:
                weibo_error = f"{type(e).__name__}: {e}"
                if progress:
                    progress.show_error(f"Weibo error: {e}")
                web_error = _append_web_error(web_error, f"Weibo: {weibo_error}")
        # Optional Douyin search in web-only mode.
        if run_douyin:
            try:
                douyin_items, douyin_error = _search_douyin(
                    topic, from_date, to_date, depth,
                )
                web_items.extend(douyin_items)
                if douyin_error and progress:
                    progress.show_error(f"Douyin error: {douyin_error}")
                if douyin_error:
                    web_error = _append_web_error(web_error, f"Douyin: {douyin_error}")
            except Exception as e:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        if run_douyin:
            douyin_future = executor.submit(
                _search_douyin, topic, from_date, to_date, depth,
            )
        ...
        if weibo_future:
            wb_timeout = timeouts.get("weibo_future", future_timeout)
            try:
                weibo_items, weibo_error = weibo_future.result(timeout=wb_timeout)
                web_items.extend(weibo_items)
                if weibo_error and progress:
                    progress.show_error(f"Weibo error: {weibo_error}")
                if weibo_error:
                ...
                weibo_error = f"Weibo search timed out after {wb_timeout}s"
                if progress:
        ...
        if douyin_future:
            dy_timeout = timeouts.get("douyin_future", future_timeout)
            try:
                douyin_items, douyin_error = douyin_future.result(timeout=dy_timeout)
                web_items.extend(douyin_items)
                if douyin_error and progress:
                    progress.show_error(f"Douyin error: {douyin_error}")
                if douyin_error:
                ...
                douyin_error = f"Douyin search timed out after {dy_timeout}s"
                if progress:
        ...
        if web_future:
            try:
                native_web_items, native_web_error = web_future.result(timeout=future_timeout)
                web_items.extend(native_web_items)
                if native_web_error and progress:
                    progress.show_error(f"Web error: {native_web_error}")
                if native_web_error:
                    web_error = _append_web_error(web_error, native_web_error)
            except TimeoutError:
                timeout_error = f"Web search timed out after {future_timeout}s"
                web_error = _append_web_error(web_error, timeout_error)
                if progress:
                    progress.show_error(timeout_error)
            except Exception as e:
                native_error = f"{type(e).__name__}: {e}"
                web_error = _append_web_error(web_error, native_error)
                if progress:
                    progress.show_error(f"Web error: {native_error}")
            sys.stderr.write(f"[web] {len(web_items)} results\n")
            sys.stderr.flush()
...
def main():
    ...
    # Auto-detect Weibo (requires SUB cookie)
    has_weibo = env.is_weibo_available(config)

    # Auto-detect Douyin native endpoint (public)
    has_douyin = env.is_douyin_available(config)
    ...
    if args.diagnose:
        ...
        diag = {
            ...
            "weibo": has_weibo,
            "weibo_auth_method": config.get("_WEIBO_SUB_SOURCE"),
            "douyin": has_douyin,
            "hackernews": True,
            "bluesky": has_bluesky,
    ...
    def _mode_from_sources(src: str) -> str:
        if src == "all":
            return "all"  # reddit + x + web
        if src == "both":
            return "both"  # reddit + x
        if src == "reddit":
            return "reddit-only"
        if src == "reddit-web":
            return "reddit-web"
    ...
    mode = _mode_from_sources(sources)
    ...
    search_run_weibo = has_weibo and qt.is_source_enabled("weibo", query_type)
    search_run_douyin = has_douyin and qt.is_source_enabled("douyin", query_type)
    ...
    if _include_sources:
        ...
        if 'weibo' in _include_sources and has_weibo:
            if not search_run_weibo:
                sys.stderr.write("[Config] INCLUDE_SOURCES override: forcing weibo\n")
                search_run_weibo = True
        if 'douyin' in _include_sources and has_douyin:
            if not search_run_douyin:
                sys.stderr.write("[Config] INCLUDE_SOURCES override: forcing douyin\n")
                search_run_douyin = True
    if args.search:
        ...
        # Explicit requests should attempt connector execution, even if preflight
        # availability is false, so users get a concrete connector error.
        search_run_weibo = "weibo" in search_sources
        search_run_douyin = "douyin" in search_sources
        include_search_web = "web" in search_sources
        # Native web-shaped connectors also populate the web bucket, so mode
        # should reflect web participation even without explicit "web" flag.
        if search_run_xiaohongshu or search_run_weibo or search_run_douyin:
            include_search_web = True
    ...
    # --search can override sources after initial mode derivation.
    mode = _mode_from_sources(sources)
    ...
    if not has_weibo:
        source_info["weibo_skip_reason"] = (
            "No WEIBO_SUB cookie - export WEIBO_SUB (and optionally WEIBO_SUBP) or enable browser cookie extraction"
        )
    if not has_douyin:
        source_info["douyin_skip_reason"] = "Douyin native endpoint unavailable"
    if not web_source:
