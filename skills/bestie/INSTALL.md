...
然后在 Claude Code 中说 `/create-bestie` 即可启动。

生成的闺蜜 Skill 默认写入 `./besties/` 目录。
...
重启 OpenClaw session，说 `/create-bestie` 启动。
...
2. 选择 **From scratch**
3. 填写 App Name（如 `bestie-skill-bot`），选择目标 Workspace → **Create App**
...
> 提示：如果你不知道目标闺蜜在哪些频道，可以先不邀请，运行采集时脚本会告知 Bot 加入了哪些频道，再补充邀请。
...
按提示粘贴 Bot Token，脚本会自动验证并保存到 `~/.bestie-skill/slack_config.json`。

配置成功后你会看到：
```
验证 Token ... OK
  Workspace：Your Company，Bot：bestie-skill-bot

✅ 配置已保存到 /Users/you/.bestie-skill/slack_config.json
```
...
```bash
cd ~/.claude/skills/create-bestie   # 或你的项目 .claude/skills/create-bestie
...
# 列出已有闺蜜 Skill
python3 tools/skill_writer.py --action list --base-dir ./besties
```
...
```
bestie-skill/        ← clone 到 .claude/skills/create-bestie/
├── SKILL.md            # skill 入口（官方 frontmatter）
├── prompts/            # 分析和生成的 Prompt 模板
├── tools/              # Python 工具脚本
├── docs/               # 文档（PRD 等）
│
└── besties/         # 生成的闺蜜 Skill 存放处（.gitignore 排除）
    └── {slug}/
