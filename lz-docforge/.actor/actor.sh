...
push_to_dataset() {
    # Example usage:
    # push_to_dataset "$RESULT_URL" "$OUTPUT_SIZE" "zip" "$PROCESS_STATUS" "$ERROR_CODE" "$MANIFEST_URL" "$DOCUMENT_URL" "$ERROR_MESSAGE"
    ...
    if [ -n "$apify_cmd" ]; then
        ...
        payload=$(jq -n \
          --arg url "$source_url" \
          --arg output_file "$result_url" \
          --arg format "$format" \
          --arg size "$size" \
          --arg status "$status" \
          --arg error_code "$error_code" \
          --arg error "$error_message" \
          --arg manifest_file "$manifest_url" \
          '{
            url: $url,
        ...
        # Use the --no-update-notifier flag if available
        if $apify_cmd --help | grep -q "\--no-update-notifier"; then
            if $apify_cmd --no-update-notifier actor:push-data "$payload"; then
            ...
            if $apify_cmd actor:push-data "$payload"; then
                echo "Successfully added record to dataset"
            else
                echo "Warning: Failed to add record to dataset"
                push_failed=1
...
generate_output_manifest() {
    ...
    local manifest_error_code="$2"
    local manifest_error_message="$3"
    local manifest_output_url="$4"
    local manifest_output_size="$5"
    local manifest_document_url="$6"
    local manifest_curl_exit_code="$7"
    ...
    jq -n \
      --arg manifest_version "1.0.0" \
      --arg actor_name "docforge-actor" \
      --arg generated_at "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
      --arg status "$manifest_status" \
      --arg error_code "$manifest_error_code" \
      --arg error_message "$manifest_error_message" \
      --arg output_key "OUTPUT" \
      --arg output_url "$manifest_output_url" \
      --arg output_content_type "application/zip" \
      --argjson output_size_bytes "${manifest_output_size:-0}" \
      --arg manifest_key "$manifest_key" \
      --arg log_key "$log_key" \
      --arg source_document_url "$manifest_document_url" \
      --argjson curl_exit_code "${manifest_curl_exit_code:-0}" \
      ...
      '{
        ...
        request: {
          http_sources: ($input_payload.http_sources // []),
...
readonly ACTOR_LOG_KEY="${DOCLING_ACTOR_LOG_KEY:-DOCFORGE_LOG}"
readonly MANIFEST_KEY="${DOCLING_ACTOR_MANIFEST_KEY:-OUTPUT_MANIFEST}"
MANIFEST_PATH="/tmp/output-manifest.json"

# Actor-level status variables
...
if ! ps -p $API_PID > /dev/null; then
    ...
    ERROR_CODE="A1001"
    ERROR_MESSAGE="docling-serve API failed to start"
    if [ -f "$API_DIR/docling-serve.log" ]; then
        echo "Full log output:"
        cat "$API_DIR/docling-serve.log"
    fi
    generate_output_manifest "$PROCESS_STATUS" "$ERROR_CODE" "$ERROR_MESSAGE" "" "0" "$DOCUMENT_URL" "0" "$MANIFEST_KEY" "$ACTOR_LOG_KEY"
    upload_to_kvs "$MANIFEST_PATH" "$MANIFEST_KEY" "application/json" "Output manifest"
...
# Define API endpoint
DOCLING_API_ENDPOINT="${DOCLING_SERVE_API_ENDPOINT:-http://localhost:5001/v1alpha/convert/source}"
echo "Using conversion endpoint: $DOCLING_API_ENDPOINT"
...
DOCUMENT_URL=$(echo "$INPUT" | jq -r '.http_sources[0].url // empty')
...
if [ -f "$API_DIR/output.zip" ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        PROCESS_STATUS="error"
        ERROR_CODE="A3002"
        ERROR_MESSAGE="Failed to upload OUTPUT zip to key-value store"
    fi

    # Build and upload output manifest
    generate_output_manifest "$PROCESS_STATUS" "$ERROR_CODE" "$ERROR_MESSAGE" "$RESULT_URL" "$OUTPUT_SIZE" "$DOCUMENT_URL" "$CURL_EXIT_CODE" "$MANIFEST_KEY" "$ACTOR_LOG_KEY"
    upload_to_kvs "$MANIFEST_PATH" "$MANIFEST_KEY" "application/json" "Output manifest"
    if [ $? -eq 0 ]; then
        MANIFEST_URL="https://api.apify.com/v2/key-value-stores/${APIFY_DEFAULT_KEY_VALUE_STORE_ID}/records/${MANIFEST_KEY}"
    else
        PROCESS_STATUS="error"
        if [ "$ERROR_CODE" = "A0000" ]; then
            ERROR_CODE="A3003"
            ERROR_MESSAGE="Failed to upload output manifest to key-value store"
        fi
    fi
else
    ...
    PROCESS_STATUS="error"
    ERROR_CODE="A3001"
    ERROR_MESSAGE="No converted output file found"
    ...
    # Build and upload error manifest
    generate_output_manifest "$PROCESS_STATUS" "$ERROR_CODE" "$ERROR_MESSAGE" "" "0" "$DOCUMENT_URL" "$CURL_EXIT_CODE" "$MANIFEST_KEY" "$ACTOR_LOG_KEY"
    upload_to_kvs "$MANIFEST_PATH" "$MANIFEST_KEY" "application/json" "Output manifest"
    if [ $? -eq 0 ]; then
        MANIFEST_URL="https://api.apify.com/v2/key-value-stores/${APIFY_DEFAULT_KEY_VALUE_STORE_ID}/records/${MANIFEST_KEY}"
    fi
...
# Push processing record to dataset
push_to_dataset "$RESULT_URL" "$OUTPUT_SIZE" "zip" "$PROCESS_STATUS" "$ERROR_CODE" "$MANIFEST_URL" "$DOCUMENT_URL" "$ERROR_MESSAGE"
if [ $? -ne 0 ]; then
    echo "Warning: dataset push failed"
fi
...
cleanup() {
    ...
    if [ -f "$LOG_FILE" ]; then
        if [ -s "$LOG_FILE" ]; then
            echo "Log file is not empty, pushing to key-value store (key: $ACTOR_LOG_KEY)..."

            # Upload log using our function
            upload_to_kvs "$LOG_FILE" "$ACTOR_LOG_KEY" "text/plain" "Log file"
        else
